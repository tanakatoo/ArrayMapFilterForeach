...
function showFirstAndLast(arr){
    let newArr = []
    arr.forEach(element => {
        newArr.push(element.charAt(0) + element.charAt(element.length-1))
    })
    return newArr
}
...
array of objects, a key, and a value and returns the array passed to the 
...
returns a new array with the value of the key with a name of "first" and the value of a key with the name of  "last" in each object, 
...
function findInObj(arr, key, searchValue) {
    const newArr = arr.filter(element => element[key] == searchValue )
    return newArr[0]
}
...
function removeVowels(str) {
    ...
    const vowels=['a','e','i','o','u']
    const returnArr = newArr.filter(function (element) {
